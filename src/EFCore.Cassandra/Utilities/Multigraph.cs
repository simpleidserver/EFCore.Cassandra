...
namespace Microsoft.EntityFrameworkCore.Utilities
{
    public class Multigraph<TVertex, TEdge> : Graph<TVertex>
    {
        ...
        public void AddEdge([NotNull] TVertex from, [NotNull] TVertex to, TEdge edge)
        {
#if DEBUG
            if (!_vertices.Contains(from))
            {
                throw new InvalidOperationException("Graph doesn't contain vertex : "+ from);
            }

            if (!_vertices.Contains(to))
            {
                throw new InvalidOperationException("Graph doesn't contain vertex : " + to);
            }
#endif
            ...
            Func<string, string> formatException = null)
        {
            ...
            var message = formatException == null ? "Circular dependency : " + cycleString : formatException(cycleString);
