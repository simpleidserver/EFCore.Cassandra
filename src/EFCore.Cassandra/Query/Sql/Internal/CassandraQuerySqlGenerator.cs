...
{
    public class CassandraQuerySqlGenerator : ThrowingExpressionVisitor, ISqlExpressionVisitor, IQuerySqlGenerator
    {
        ...
        public CassandraQuerySqlGenerator(QuerySqlGeneratorDependencies dependencies, SelectExpression selectExpression)
        {
        ...
        private class InExpressionValuesExpandingVisitor : RelinqExpressionVisitor
        {
            ...
            private IReadOnlyList<Expression> ProcessInExpressionValues(IEnumerable<Expression> inExpressionValues)
            {
                        ...
                        case ParameterExpression parameterExpression:
                            if (_parametersValues.TryGetValue(parameterExpression.Name, out var parameterValue))
                            {
                                if (parameterValue == null
                                    && typeof(System.Collections.IEnumerable).IsAssignableFrom(parameterExpression.Type)
                                    && parameterExpression.Type != typeof(string)
                                    && parameterExpression.Type != typeof(byte[]))
                                {
                                    throw new InvalidOperationException(RelationalStrings.ExpectedNonNullParameter(parameterExpression.Name));
                                }
            ...
            {
                if (value is System.Collections.IEnumerable valuesEnumerable
            ...
            private IReadOnlyList<Expression> ExtractNonNullExpressionValues(IEnumerable<Expression> inExpressionValues)
            {
        ...
        protected virtual void GenerateList(IReadOnlyList<Expression> items,
            Action<IRelationalCommandBuilder> joinAction = null,
            IReadOnlyList<RelationalTypeMapping> typeMappings = null)
